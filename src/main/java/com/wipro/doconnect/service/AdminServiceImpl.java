...
public class AdminServiceImpl implements IAdminService {
	...
	public Question approveQuestion(Long questionId) {
		...
		for (Admin admin : admins) {
			sendMail(admin.getEmail(), "Question Added", "Question added successfully");
		}
	...
	public Answer approveAnswer(Long answerId) {
		...
		// a mail should go to the list of admins that a answer is published
		List<Admin> admins = adminRepo.findAll();
		for (Admin admin : admins) {
			sendMail(admin.getEmail(), "Answer Publised", "Answer added successfully");
		}
	...
	public Boolean sendMail(String emailId, String type, String type1) {
		try {
			emailSenderService.sendEmail(emailId, type, type1);
			return true;
		} catch (Exception e) {
